...
export function TeamConstraints({
  players,
  constraints,
  onConstraintsChange,
}: TeamConstraintsProps) {
  const { t } = useTranslation();
  const [firstPlayer, setFirstPlayer] = useState("");
  const [secondPlayer, setSecondPlayer] = useState("");
  const [constraintType, setConstraintType] = useState<"together" | "separate">(
    "together"
  );
  ...
  const handleAddConstraint = () => {
    if (firstPlayer && secondPlayer && firstPlayer !== secondPlayer) {
      ...
      const existingConstraint = constraints.find(
        (constraint) =>
          (constraint.playerId1 === firstPlayer &&
            constraint.playerId2 === secondPlayer) ||
          (constraint.playerId1 === secondPlayer &&
            constraint.playerId2 === firstPlayer)
      );
  ...
  const availablePlayersForFirst = players.filter(
    (player) => player.id !== secondPlayer
  );
  const availablePlayersForSecond = players.filter(
    (player) => player.id !== firstPlayer
  );
              ...
              <div className="flex items-center gap-4 justify-between">
                <div className="space-y-2 grow">
                  ...
                  <Select value={firstPlayer} onValueChange={setFirstPlayer}>
                    <SelectTrigger className="w-full">
                      <SelectValue
                        placeholder={t("constraints.selectPlayer")}
                      />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value: "together" | "separate") =>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-medium">
                          {getPlayerName(constraint.playerId1)}
                        </span>
                        <span className="text-muted-foreground">and</span>
                        <span className="font-medium">
                          {getPlayerName(constraint.playerId2)}
                        </span>
                      </div>
